...
async function handleSignup(req, res) {
  ...
  try {
    ...
    res.redirect("back"); // Adjust as needed
  } catch (error) {
  ...
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: {
        ...
        pass: GOCAR_APP_PASSWORD // App-specific password
      },
    });
...
async function handleMyBookingsPage(req, res) {
  try {
      ...
      loggedInUser.BookingDetails.map(async (booking) => {
        try {
          ...
          const carDetail = await product
            ...
            .select(
              "-sliderImage -carParkedAt -createdAt -updatedAt -carDftDetail"
            )
            .exec();
    ...
    res.redirect("back"); // Redirect to the previous page in case of an error
  }
}
...
async function handleMyWishlistPage(req, res) {
  try {
    ...
    const wishlistedCars = await product
      ...
      .select("-sliderImage -carParkedAt -createdAt -updatedAt")
      .exec();
