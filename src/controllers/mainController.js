...
async function handleRazorpayPaymentVerification(req, res) {
  ...
  try {
    const generatedSignature = crypto
      .createHmac('sha256',secret ) // Use environment variable for Razorpay secret
    ...
    if (generatedSignature === razorpay_signature) {
      ...
      // Redirect to payment status with failure
...
async function handlePaymentStatusPage(req, res) {
  ...
  if (!req.session.bookingData || !req.session.carId) {
    req.flash("error", "Something went wrong.");
  ...
  const bookingInfo = {
    ...
    address: req.session.bookingData.address || "N/A",
    date: req.session.bookingData.date || "N/A",
    time: req.session.bookingData.time || "N/A",
